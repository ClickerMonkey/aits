 ...
 */
export type Extend<TBase, TOverride> = TBase & TOverride;
/**
 * T plus any extra properties.
 */
export type Plus<T> = T & {
    [P in PropertyKey]: P extends keyof T ? T[P] : any;
};
/**
 ...
 */
export type OptionalParams<P> = P extends [] ? [] : P extends [infer F] ? RequiredKeys<F> extends never ? [param?: F] : [param: F] : P extends [...infer F, infer R] ? RequiredKeys<R> extends never ? [...OptionalParams<F>, param?: R] : [...F, R] : never;
/**
...
*/
export interface Component<TContext = {}, TMetadata = {}, TName extends string = string, TInput extends object = {}, TOutput = string, TRefs extends Tuple<ComponentCompatible<TContext, TMetadata>> = []> {
    /**
    ...
    */
    run<TRuntimeContext extends TContext, TRuntimeMetadata extends TMetadata, TCoreContext extends Context<TRuntimeContext, TRuntimeMetadata>>(...[input, ctx]: OptionalParams<[TInput, TCoreContext]>): TOutput;
    /**
     * Determines if the component is applicable in the given context.
     *
     * @param ctx - The context to check applicability against.
     */
    applicable<TRuntimeContext extends TContext, TRuntimeMetadata extends TMetadata, TCoreContext extends Context<TRuntimeContext, TRuntimeMetadata>>(...[ctx]: OptionalParams<[TCoreContext]>): Promise<boolean>;
}
/**
...
export type ComponentTuple<TContext, TMetadata, TComponents extends Tuple<AnyComponent>> = TComponents & {
    [K in keyof TComponents]: TComponents[K] extends Component<infer U, infer V, any, any, any, any> ? TContext extends U ? TMetadata extends V ? TComponents[K] : never : never : never;
};
/**
 * A type representing any AI component that is compatible with the given context and metadata.
 */
export type ComponentCompatible<TContext, TMetadata> = AnyComponent extends infer R ? R extends Component<infer U extends TContext, infer V extends TMetadata, any, any, any, any> ? Component<U, V, any, any, any, any> : never : never;
/**
 ...
 */
export type ComponentsAll<C extends AnyComponent> = C extends Component<infer t0, infer t1, infer t2, infer t3, infer t4, infer TRefs> ? C | TRefs[number] | ComponentsAll<TRefs[number]> : never;
/**
 * Extracts the names of a tuple of components.
 */
export type Names<T extends Tuple<AnyComponent>> = T extends Tuple<infer C extends Component<infer t0, infer t1, infer TName, infer t2, infer t3, infer t4>> ? TName : never;
/**
 ...
 */
export type Executor<TContext, TMetadata> = <TRuntimeContext extends TContext, TRuntimeMetadata extends TMetadata>(request: Request, context: TRuntimeContext, metadata?: TRuntimeMetadata, signal?: AbortSignal) => Promise<Response>;
/**
 ...
 */
export type Streamer<TContext, TMetadata> = <TRuntimeContext extends TContext, TRuntimeMetadata extends TMetadata>(request: Request, context: TRuntimeContext, metadata?: TRuntimeMetadata, signal?: AbortSignal) => AsyncGenerator<Chunk, Response>;
/**
 ...
 * @returns
 */
export type Runner = <TComponent extends AnyComponent, TContext extends ComponentContext<TComponent>, TMetadata extends ComponentMetadata<TComponent>, TCoreContext extends Context<TContext, TMetadata>>(component: TComponent, input: ComponentInput<TComponent>, ctx: TCoreContext, getOutput: (ctx: TCoreContext, events?: Events<TComponent>) => ComponentOutput<TComponent>) => ComponentOutput<TComponent>;
/**
...
*/
export type ResponseFormat = 'text' | 'json' | z.ZodType<object, object>;
/**
 ...
 */
export interface Events<TRoot extends AnyComponent, TNodes extends ComponentsAll<TRoot> = ComponentsAll<TRoot>> {
