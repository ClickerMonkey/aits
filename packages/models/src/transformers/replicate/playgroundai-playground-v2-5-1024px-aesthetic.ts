...
type Schemas = {
  ...
  scheduler: "DDIM" | "DPMSolverMultistep" | "HeunDiscrete" | "K_EULER_ANCESTRAL" | "K_EULER" | "PNDM" | "DPM++2MKarras" | "DPMSolver++";
};
      ...
      imageGenerate: {
        convertRequest: async (request, ctx): Promise<Schemas["Input"]> => {
          ...
          if (request.size) {
            ...
            if (parts.length === 2) {
              input.width = parseInt(parts[0]);
              input.height = parseInt(parts[1]);
            }
          }
          return input;
        },
