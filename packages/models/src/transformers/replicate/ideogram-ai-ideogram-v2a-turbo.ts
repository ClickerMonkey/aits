    ...
    const transformer: ReplicateTransformer = {
      imageGenerate: {
        convertRequest: async (request, ctx): Promise<Schemas["Input"]> => {
          ...
          // Map standard 'size' to aspect_ratio or resolution if provided and valid
          if (request.size && !input.resolution && !input.aspect_ratio) {
            if (request.size.includes(':')) {
              // Simple check, strict validation happens at runtime/API level
